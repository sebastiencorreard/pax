...
\begin{document}
    ...
    \begin{center}
	\noun{\LARGE The Galton box [quincunx or bean machine]} \
	
	
	\noun{Empirical and theoretical probability}
	
    \end{center}
    \footnote{\includegraphics[  width=1cm, height=1cm]{./plaatjes/linux.jpg}                                 
    \emph{used} $LinuX$ \emph{and} \LaTeX 
...
\begin{tabular}{|l|}
\hline
In a Galton box we drop at least 65 marbles in 6 bins.\tabularnewline 

The probability for a marble to drop at the right side of a pin is 0.3.\tabularnewline

Determine:\tabularnewline

- the empirical probability \tabularnewline

- the theoretical probabilty \tabularnewline
...
The empirical probability is the experimental probability.\\
And can only be determined after the actual experiment.\\
In this example we drop 65 marble in the quincunx.\\
...
The empirical probability can be calculated with:

\indent

\begin{tabular}{|l c c c c c c |}
    \hline
    Bin number  & 1 & 2 & 3 & 4 & 5 & 6\tabularnewline
    \hline
    marbles  & 15 & 17 & 22 & 10 & 1 & 0\tabularnewline
    \hline
     & & & & & &  \tabularnewline
    Empirical P & $\frac{15}{65}$&$\frac{17}{65}$ &$\frac{22}{65}$ & $\frac{10}{65}$&$\frac{1}{65}$ & 0   \tabularnewline
    \hline
\end{tabular}
...
To determine the theoretical probability we do not need the actual experiment.

You just have to count the number of 'decisions' a marble can make between 'dropping to the left' and
'dropping to the right' .\\
In the Galton box the marble must make 5 decisions.\\
For a marble to drop in bin 6 ,it needs to drop 5 times to the right side of a pin.

\indent

\begin{multicols}{2}
Let $ X $ be the number of 'drop to the right'.
$ X $ is a binomial distribution with $ n = 5 $ and $ p = 0.3 $
...
\begin{tabular}{|l c c c c c c |}
    ...
    Bin number  & 1 & 2 & 3 & 4 & 5 & 6\tabularnewline
    \hline
    Drop to the right  & 0 & 1 & 2 & 3 & 4 & 5 \tabularnewline
    \hline 
    Theoretical P & 0.1681 & 0.3602 & 0.3087 & 0.1323 & 0.0284 & 0.0024 \tabularnewline
    \hline 
\end{tabular}

\indent

THe theoretical probability can be calculated with a graphical calculator:\\

y1=binompdf(5,0.3,X) \\

[in this example , we used 'Texas Instruments TI83' ] \\

The probablities can be read from the table

\indent

Below is an image of the actual WIMS exercise.
