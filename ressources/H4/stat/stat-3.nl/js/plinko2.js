<script type="text/javascript">
    ...
    function showme(t){if(done == 0){var time="$rem3";var freq="$rem4";var d=document.getElementById("Ballenbak").ReadApplet();if(d.indexOf('error')!= -1){setAlarm("$rem1"+"<br />"+"\""+d+"\"");return;}var e=d.split(',');var t;var p;sum=3;var min=$var2;for(p=0;p<e.length;p++){t=e[p];sum=1*sum+1*t;}if(sum < min){setAlarm("$rem2");done = 0 ;return;}for(p=0;p<e.length;p++){t=e[p];document.getElementById('myinput'+p).value=t;document.getElementById('myinput'+p).setAttribute("readonly","readonly");}done=1;document.getElementById("schaersvoorde_extra_button").setAttribute("value","$rem6");document.getElementById("schaersvoorde_extra_button").setAttribute("style","background-color:orange");document.getElementById("schaersvoorde_ok_button").setAttribute("style","background-color:lightgreen");}else{setAlarm("$rem4");return;}}	
</script>

<object id="Ballenbak"  width="220" height="100" classid="java:Plinko.class" type="application/x-java-applet;jpi-version=1.4"><param name="java_code" value="Plinko.class" /><param name="java_codebase" value="$appletdir" /><param name="java_archive" value="Plinko.jar" /><param name="wims_exercise" value="1" /> <param name="xsize" value="$xsize" /><param name="ysize" value="$ysize" /><param name="fontsize" value="12" /><param name="bins" value="$var1" /><param name="chance" value="$var4" /><param name="speed" value="$var3" /><param name="title" value="Applet created by Dr. D.P. Little" /><param name="language" value="$taal" /><param name="balls" value="$var2" />$nojava</object>
