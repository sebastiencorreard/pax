...
ui.api_path = '/';
ui.base_url = '';
ui.wims_no_warning = true;/* jm.evers */
ui.scale = 40;
...
ui.initTemplates = function (){
    /* the members of array wims_templates are already checked on 'V2000' and 'M END' */
    //alert("wims_templates.length="+wims_templates.length);
    rnd.templates = [];
    if(wims_templates == null || typeof(wims_templates) == 'undefined'){ return; };
    if( wims_templates.length > 0 ){
     ui.template_tool_modes.clear();
     var tbody = $('template_dropdown_list').select('table > tbody')[0];tbody.update();
     var idx = 0;
     for( var p = 0; p < len ; p++ ){
      var my_mol_file = wims_templates[idx];
      var end_idx = my_mol_file.indexOf('M  END');
      var my_mol_file = my_mol_file.substring(0,end_idx+6);
      var my_name_ = wims_templates[idx].split('\n');
      my_name = (my_name_[0]).toString(); /* actual filename */
      var tmpl = { name:my_name, molfile: my_mol_file, aid:0, bid:0 };
      rnd.templates.push(tmpl);
      ui.template_tool_modes.push('template_' + idx);
      tbody.insert('<tr class="dropdownListItem" id="template_'+idx+'" title="'+tmpl.name+' (press t)">'+'<td><div id="template_'+idx+'_preview" style="float:right"><img style="align:right;width:22px;height:22px;" class="dropdownIconTemplate" src="'+ui.api_path+'icons/png/template/template0.sidebar.png" alt="" /></div></td><td>'+tmpl.name+'</td></tr>');
    idx++;
    };
   }
   else
   { 
    rnd.templates.push(rnd.Editor.benzene);
   };
};
...
ui.init = function (parameters, opts)
{
    ...
    if (!this.standalone){ this.initTemplates();}
    ...
    // Client area events
     this.client_area= $('clientArea');
    this.client_area.observe('scroll', ui.onScroll_client_area);
...
ui.keyboardActions = {
    ...
    sgroup_tool: function() { ui.onClick_SideButton.call($('wims_sgroup')); },
    cleanup_tool: function() { ui.onClick_CleanUp.call($('wims_clean_up')); },
    ...
    template_tool: function() { if (ui.template_tool_modes.length < 1) return; ui.onMouseDown_DropdownListItem.call($(util.listNextRotate(ui.template_tool_modes, ui.mode_id))); },
...
ui.loadMoleculeFromInput = function ()
{
    if (!util.strip($('input_mol').value)) {
        alert("The input field is empty. Please enter a structure in SMILES or MOLFile/RXNFile format.");
        return;
    }
    ui.hideDialog('open_file');
...
ui.onScroll_client_area = function(event)
{
