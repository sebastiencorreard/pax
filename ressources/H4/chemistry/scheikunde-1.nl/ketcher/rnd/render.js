...
rnd.RenderDummy = function (){
    client_area = document.getElementById('answerArea');
    client_area.innerHTML = "";
    this.answerpaper = new Raphael(client_area);
    this.answerpaper.rect(0, 0, 100, 100).attr({
		'fill':'#0F0',
		'stroke':'#ff0000'
    });
...
rnd.Render = function (client_area, scale, opt, viewSz){
	this.opt = new rnd.RenderOptions(opt);
//	if( typeof(viewSz) != 'object'){ viewSz = new util.Vec2(321,334);};
	this.useOldZoom = Prototype.Browser.IE;
	this.scale = scale || 100;
	this.baseScale = this.scale;
	this.offset = new util.Vec2();
	this.client_area = client_area = $(client_area);
	client_area.innerHTML = "";
	this.paper = new Raphael(client_area);
	this.size = new util.Vec2();
	this.viewSz = viewSz || new util.Vec2(client_area['clientWidth'] || 100, client_area['clientHeight'] || 100);
	...
	this.client_areaPos = new util.Vec2(valueL, valueT);
	rnd.canvas =  this.paper;
    ...
    //BEGIN
    client_area.observe('selectstart', function(event) {
    ...
    client_area.observe('touchstart', function(event) {
    ...
    client_area.observe('touchmove', function(event) {
        self.resetLongTapTimeout(true);
    ...
    client_area.observe('gesturestart', function(event) {
        this._tui = this._tui || {};
        this._tui.scale0 = ui.render.zoom;
        event.preventDefault();
    });
    client_area.observe('gesturechange', function(event) {
        ui.setZoomStaticPoint(this._tui.scale0 * event.scale, ui.page2canvas2(this._tui.center));
        ui.render.update();
        event.preventDefault();
    });
    client_area.observe('gestureend', function(event) {
        delete this._tui;
        event.preventDefault();
    });
    //END

	client_area.observe('onresize', function(event) {
        render.onResize();
    });
    ...
    if ('hiddenPaths' in rnd.ReStruct.prototype) {
        client_area.observe('touchend', function(event) {
...
rnd.Render.prototype.findItem = function(event, maps, skip) {
    var ci = this.findClosestItem(
        'ui' in window && 'page2obj' in ui
            ...
            : new util.Vec2(event.pageX, event.pageY).sub(this.client_areaPos),
        maps,
        skip
    );
...
{
	this.setViewSize(new util.Vec2(this.client_area['clientWidth'], this.client_area['clientHeight']));
};
