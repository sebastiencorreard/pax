target=effcum1,effcum2,effcum3,effcum4
#define TITRE Effectifs cumulés
#include "author.inc"

#if #TARGET(effcum1)
 \title{TITRE 1}
 \text{donnees=wims(randrecord src/stat2.don)}
 \text{tmp=row(1,\donnees)}
 \integer{eff=randint(\tmp[1]..\tmp[2])}
 \text{tmp=row(2,\donnees)}
 \integer{min=randint(\tmp[1]..\tmp[2])}
 \text{tmp=row(3,\donnees)}
 \integer{max=randint(\tmp[1]..\tmp[2])}
 \text{enonce=row(4,\donnees)}
 \text{nameclass=row(5,\donnees)}
 \text{question=Calculer les effectifs cumulés croissants.}
 \text{et=wims(values v for v=\min to \max)}
 \integer{type=1}
 \integer{nb=\max-\min+1}
 \integer{supertype=0}
#endif
#if #TARGET(effcum2)
 \title{TITRE 2}
 \text{donnees=wims(randrecord src/stat2.don)}
 \text{tmp=row(1,\donnees)}
 \integer{eff=randint(\tmp[1]..\tmp[2])}
 \text{tmp=row(2,\donnees)}
 \integer{min=randint(\tmp[1]..\tmp[2])}
 \text{tmp=row(3,\donnees)}
 \integer{max=randint(\tmp[1]..\tmp[2])}
 \text{enonce=row(4,\donnees)}
 \text{nameclass=row(5,\donnees)}
 \text{question=Calculer les effectifs cumulés décroissants.}
 \text{et=wims(values v for v=\min to \max)}
 \integer{type=2}
 \integer{nb=\max-\min+1}
 \integer{supertype=0}
#endif

#if #TARGET(effcum3)
 \title{TITRE 3}
 \text{donnees=wims(randrecord src/stat3.don)}
 \text{tmp=row(1,\donnees)}
 \integer{eff=randint(\tmp[1]..\tmp[2])}
 \integer{int=row(4,\donnees)}
 \text{tmp=row(2,\donnees)}
 \integer{min=randint(\tmp[1]..\tmp[2])*\int}
 \text{tmp=row(3,\donnees)}
 \integer{max=randint(\tmp[1]..\tmp[2])*\int}
 \text{enonce=row(5,\donnees)}
 \text{nameclass=row(6,\donnees)}
 \text{name=row(7,\donnees)}
 \text{question=Calculer les effectifs cumulés croissants.}
 \integer{nb=(\max-\min+1)/\int}
 \text{et=}
 \for{i=1 to \nb}{
  \integer{em=\min+(\i-1)*\int}
  \integer{eM=\min+\i*\int}
  \text{et=wims(append item \(\em \leq \name < \eM) to \et)}
 }
 \integer{type=1}
 \integer{supertype=1}
#endif

#if #TARGET(effcum4)
 \title{TITRE 4}
 \text{donnees=wims(randrecord src/stat3.don)}
 \text{tmp=row(1,\donnees)}
 \integer{eff=randint(\tmp[1]..\tmp[2])}
 \integer{int=row(4,\donnees)}
 \text{tmp=row(2,\donnees)}
 \integer{min=randint(\tmp[1]..\tmp[2])*\int}
 \text{tmp=row(3,\donnees)}
 \integer{max=randint(\tmp[1]..\tmp[2])*\int}
 \text{enonce=row(5,\donnees)}
 \text{nameclass=row(6,\donnees)}
 \text{name=row(7,\donnees)}
 \text{question=Calculer les effectifs cumulés décroissants.}
 \integer{nb=(\max-\min+1)/\int}
 \text{et=}
 \for{i=1 to \nb}{
  \integer{em=\min+(\i-1)*\int}
  \integer{eM=\min+\i*\int}
  \text{et=wims(append item \(\em \leq \name < \eM) to \et)}
 }
 \integer{type=2}
 \integer{supertype=1}
#endif

\text{E=slib(generator \nb,\eff)}
\text{nametype=item(\type,Effectifs cumulés croissants,Effectifs cumulés décroissants)}

\integer{to=\type=1?0:\eff}
\text{sg=\type=1?+:-}
\text{res=\to}
\for{k=1 to \nb}{
 \integer{to=\to\sg\E[\k]}
 \text{res=\res,\to}
}
\text{res=\type=1?item(2 to -1,\res)}

\text{r=item(1 to \nb,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10)}
\steps{\r}

\statement{
\enonce<br>
<p>\question ?</p>
  <table class="wimscenter wimsborder">
   <tr>
    <th>\nameclass</th>
    \for{l=1 to \nb}{<td>\et[\l]</td>}
   </tr><tr>
    <th>Effectifs</th>
    \for{l=1 to \nb}{<td>\E[\l]</td>}
   </tr><tr>
    <th>\nametype</th>
    \for{l=1 to \nb}{<td>\embed{\r[\l],5}</td>}
   </tr>
  </table>
}

\answer{1}{\res[1]}{type=numexp}
\answer{2}{\res[2]}{type=numexp}
\answer{3}{\res[3]}{type=numexp}
\answer{4}{\res[4]}{type=numexp}
\answer{5}{\res[5]}{type=numexp}
\answer{6}{\res[6]}{type=numexp}
\answer{7}{\res[7]}{type=numexp}
\answer{8}{\res[8]}{type=numexp}
\answer{9}{\res[9]}{type=numexp}
\answer{10}{\res[10]}{type=numexp}
