target= oefegal3 oefegal4
#define TITRE Compléter
#if defined TARGET_oefegal3
   \title{TITRE 3}
   \text{signe=+}
#endif

#if defined TARGET_oefegal4
   \title{TITRE 4}
   \text{signe=-}
#endif
...
\if{\ch=1}{\text{rep=\m1,\repco[1],\tmp2}}
 {\text{rep=\m2,\repco[1],\tmp2}}
...
\embed{r2,\size} \signe \(\tmp1) + \embed{r3,\size}}
{(\(\m1) \signe \embed{r1,\size})\(^2) =
\embed{r2,\size} \signe \(\tmp1) + \embed{r3,\size}}
</div>
}

\answer{}{\rep[1]}{type=litexp}
\answer{}{\rep[2]}{type=litexp}
\answer{}{\rep[3]}{type=litexp}
