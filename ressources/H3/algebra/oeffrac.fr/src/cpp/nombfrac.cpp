target=adnbfr sonbfr munbfr dinbfr
...
#include "lang.inc"
#if defined TARGET_adnbfr
 \text{ope=+}
 \text{opetex=+}
#endif
#if defined TARGET_sonbfr
 \text{ope=-}
 \text{opetex=-}
#endif
#if defined TARGET_munbfr
 \text{ope=*}
 \text{opetex= \times }
#endif

#if defined TARGET_dinbfr
 \text{ope=/}
 \text{opetex= : }
#endif
...
\for{i=1 to \confparm1}{
 ...
 \integer{a=randitem(\sg)*randint(2..20)}
 ...
 \integer{nch=randitem(1,2)}
 \if{\nch=1}{
   \text{don=\a \opetex \frac{\b}{\c},(\a)\ope((\b)/(\c))}
 }{
   \text{don=\a<0?\frac{\b}{\c} \opetex (\a),((\b)/(\c)) \ope (\a):\frac{\b}{\c} \opetex \a,((\b)/(\c)) \ope (\a)}
 }
 \text{eno=item(1,\don)}
 \text{sol=item(2,\don)}
 \text{sol=maxima(\sol;)}
