...
#if defined TARGET_qcmfonct
\title{Qual è la funzione?}
...
\text{question=Qual è la funzione \(f\) la cui rappresentazione grafica è qui sotto?
    Indicare una espressione di \(f(x)\).}
\text{nom=f(x)}
#endif

#if defined TARGET_qcmequa
\title{Qual è l'equazione?}
\text{good=\(y= \a x\B)}
\text{bad=\(y= \b x\A),\(y= \oa x\B),\(y= \oa x\OB),\(y= \a x\OB),\(y= \b x\OA)}
\text{question=Qual è l'equazione della retta qui sotto?}
\text{nom=y}
#endif
