...
\integer{nb1=(randint(110000..990000)/10^(5-\g))}
\integer{nb2=(randint(110000..990000)/10^(5-\g-\e))}
...
\text{c3=wims(char 1 to 1 of \rep)}
\text{cc3=wims(char 2  to 2 of \rep)}

\if{\cc3>4}{\integer{c3=\c3+1}}
\text{list3=}
%% calcul : le plus proche de la liste3.
%% ne faudrait-il pas regarder le plus proche des listes 1, 2 et 3
%% puisqu'elles sont présentées ensemble ??
\text{indice=1}
\real{testrep=1000000000}
\for{i=1 to 7}{
  \text{v=item(\i,\list)}
  \real{v=\v*\c3}
  \real{testrep1=abs(\v-\rep)}
  \if{\testrep1<\testrep}{\text{indice=\i}}
  \real{testrep=\testrep1}
  \text{v=slib(numeration/ecriturenombre \v)}
  \text{list3=wims(append item \v to \list3)}
}
%% ce calcul est supprimé.
if{\e=0 and \c1+\c2>9}{\integer{e=\e+1+\ee}}
if{\e<0}{\text{rep3=\list3[\g+\gg]}}{\text{rep3=\list3[\e+\g+\ee+\gg]}}

\text{rep3=\list3[\indice]}
