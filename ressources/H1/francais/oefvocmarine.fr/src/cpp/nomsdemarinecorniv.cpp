target=nomsdemarinecorniv1 nomsdemarinecorniv2 nomsdemarinecorniv3 nomsdemarinecorniv4

#define TITRE Les noms de la marine.
#if defined TARGET_nomsdemarinecorniv1
\title{TITRE Niveau facile.}
\text{data=wims(record 1 of nomsdemarinecorniv1)}
#endif
#if defined TARGET_nomsdemarinecorniv2
\text{data=wims(record 1 of nomsdemarinecorniv2)}
\title{TITRE Niveau moyen.}
#endif
#if defined TARGET_nomsdemarinecorniv3
\text{data=wims(record 1 of nomsdemarinecorniv3)}
\title{TITRE Niveau difficile.}
#endif
#if defined TARGET_nomsdemarinecorniv4
\text{data=wims(record 1 of nomsdemarinecorniv4)}
\title{TITRE Niveau excellent.}
#endif

%%Le source de cet exercice a été créé à l'aide de Quicktool
%%Le type de l'exercice est CORRESP.
%% Vous pouvez le modifier si vous désirez l'adapter.
%% Mon Jun 15 22:14:53 2009
%%Mise a jour 8 mai 2011
...
\css{<style>
   .question {background-color: #D7F44A ;
   margin: 2% 2%;padding: 1%; text-align:center}
   .reponse {background-color: #FFCC99;
   color: black;margin: 2% 2%;padding: 0%;}
 </style>}
\language{fr}
\computeanswer{no}
\format{html}
...
\text{instruction=asis(Saurez-vous associer les noms synonymes ?)}
\text{instruction2=asis(Accès à un lexique pour les mots)}
\text{choice=}
\text{common=}
\text{audio_file=no}
\text{reading_file=no}
...
\text{gauche=wims(embraced randitem \gauche)}
\text{gauche=wims(replace internal | by \ \  in \gauche)}
\text{helpmot=}
\for{m=1 to \M}{
  \text{mot=wims(word 2 of \gauche[\m])}
  \text{mot=\mot iswordof un une le la de du les des d' ! -? wims(word 1 of \gauche[\m])}
  \text{helpmot=wims(append item <a target="wims_external" href="http://www.cnrtl.fr/definition/\mot">\mot</a>
   to \helpmot)}
}

\statement{
\nopaste \instruction \preliminaire
  \if{\reading notsametext }{\special{help reading,[à lire]}}
  \if{\image notsametext }{\special{help image,[à regarder]}}
  \audio\video
  <div class="question">\embed{reply1,\size}</div>
  \if{\confparm2=1}{ \instruction2 \for{mm=1 to \M}{\special{help aide,\helpmot[\mm]}}}
}

\answer{}{\gauche; \droite}{type=correspond}{option=\partialscore}

\feedback{\sc_reply1 <1 }{
\instruction2 \for{mm=1 to \M}{\special{help aide,\helpmot[\mm]}}
}
