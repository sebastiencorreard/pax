target = tan3 tan4 tan3e tan4e

\language{fr}
\range{-5..5}
\author{Chantal, Causse}
\email{Chantal.Causse@noos.fr}
\computeanswer{yes}
\format{html}
\precision{10000}

#if defined TARGET_tan3
\title{Tangente à une courbe (avec exp)}
#endif
#if defined TARGET_tan3e
\title{Tangente à une courbe (avec exp) Etapes}
#endif
#if defined TARGET_tan4
\title{Tangente à une courbe (avec exp en 0)}
\author{Chantal, Causse;Fabien,Sommier}
#endif
#if defined TARGET_tan4e
\title{Tangente à une courbe (avec exp en 0) Etapes}
#endif

\real{a=randint(1..4)*random(-1,1)}
\real{b=randint(1..5)*random(-1,1)}
\real{c=randint(2..5)}
\real{d=randint(1..7)}
\rational{q=(1-\d)/\c}

\integer{choix=randint(1..2)} 

\function{lin = simplify(\b*x+\c)}

\function{p=item(\choix,
(\lin)*e^(\a*x),
\lin + \d*e^(\a*x)
)}
\function{p=wims(\p)}
\text{tp=texmath(\p)}

\function{der=diff(\p,x)}

function{der=simplify(\der)}
\text{dermath=texmath(\der)} (pour la solution)
\if{\choix=1}{ !!! cela pour presenter la derivee sous forme factorisee
\function{fact=(\der)/(e^(\a*x))}
\function{fact=maxima(expand(\fact))}
\text{der=(\fact)*(e^(\a*x))}}

#if defined TARGET_tan3 || defined TARGET_tan3e 
\text{x0=item(\choix,
0,
random(0,0,-1,1),
random(\q,\q,0),
\q,
\q
)}
#else
\text{x0=0}
#endif



\text{sol=evalue(\der,x=\x0)}
\function{solmath=maxima(expand(\sol))}
\text{sol2=evalue(\p,x=\x0)}
\function{tan=(\sol)*(x-(\x0))+(\sol2)}
\function{tans=maxima(expand(\tan))}
\text{a=\tans}
\text{a=evalue(\a,e=y)}
\text{b1=pari(a=\a;polcoeff(a,1,x))}
\text{b1=wims(replace internal y by e in \b1)}
\function{b1=maxima(expand(\b1))} 
\text{b2=pari(a=\a;polcoeff(a,0,x))}
\text{b2=wims(replace internal y by e in \b2)}
\function{b2=maxima(expand(\b2))} 

\text{b=(\b1)*x+\b2}
\text{bm=texmath(\b)}

#if defined TARGET_tan3e || defined TARGET_tan4e
\steps{reply1
reply2,reply3,reply4}
\statement{<p>Soit \(f(x) = \tp).</p>
\if{\step = 1}{
<p>Le but de l'exercice est de déterminer l'équation réduite de la tangente \(T) à la courbe représentant \(f) au point \(A) d'abscisse \x0.</p>
<p>Pour cela, il faut commencer par calculer \(f'(x)).</p>}
{<ul>
<li>\(f'(x) = \)  \reply1 </li>
<li>Déterminer l'équation réduite de la tangente \(T\) à la courbe représentant \(f\) au point \(A\) d'abscisse \x0.</li>
<li>Donner les coordonnées d'un point \(B\) de la tangente \(T\) autre que \(\A).</li>
</ul>
}


}


#else 
\statement{
Soit \(f(x) = \tp).
<ul>
<li>Calculer \(f'(x)\).</li>
<li>Déterminer l'équation réduite de la tangente \(T\) à la courbe représentant \(f\) au point \(A\) d'abscisse \x0.</li>
<li>Donner les coordonnées d'un point \(B\) de la tangente \(T\) autre que \(A\).</li>
</ul>}
#endif



\hint{
Calculer \(f(\x0)) et  \(f'(\x0))<br />
<span class="wims_emph"> 
La tangente passe par le point de coordonnées \((\x0 ; f(\x0))\) et a comme coefficient directeur \(f'(\x0)\)</span>
}

\answer{\(f'(x))}{\der}{type = function} 
\answer{Equation de la tangente : \(y )}{\tans}{type = function} 
\answer{Abscisse de \(B)}{\rep4}{}
\answer{Ordonnée de \(B)}{\rep5}{}
\condition{\(B) est sur la droite \(T)}{\rep5 = \sol*(\rep4-\x0)+\sol2}
\condition{\(B) est différent de \(A)}{\rep4 <> \x0}

\solution{
\(f'(x)= \dermath).<br />
On en déduit que
\(f'(\x0)=\solmath) puis que la tangente au point d'abscisse \(\x0) a pour équation \(y = \bm)<br />
Pour les coordonnées de \(B), on choisit pour \(x_B) une valeur de \(x) différente de \(\x0) et on calcule l'ordonnée de \(B) en remplaçant \(x) par \(x_B) dans l'équation réduite de \(T).


}
