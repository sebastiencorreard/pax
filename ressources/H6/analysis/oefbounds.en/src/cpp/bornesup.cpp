...
#if defined TARGET_bornesup1
\matrix{mhyp=\(A) is bounded above by \a,0,0,0,1,0,0
...
\(\a\in A),0,0,0,0,0,1
\(A) is bounded below by \a,0,0,0,1,0,0
...
\(\sup(A)= \a)
 \(\sup(A)\leq \a)
\(\sup(A)< \a)
the statement does not make it possible to know if \(\sup(A)) exists
\(\inf(A)\leq \a)
\(\inf(A)<\a)
\(\inf(A)= \a)
 \(\inf(A)\geq \a)
\(\inf(A)> \a)
the statement does not make it possible to know if \(\inf(A)) exists
}
#endif
#if defined TARGET_bornesup2
...
%liste des réponses
\matrix{rep=\(A) is bounded below by \a
\(A) is bounded above by \a
...
\statement{
  <div class="oef_explain">Let \(A) be a non-empty set.
We denote by \(\sup(A)) its supremum (least upper bound) and \(\inf(A)) its infimum (greatest lower bound)
if they exist.
We know that:
