...
\text{size=40x60x1}
\text{ch=item(EQCNT,两个, 两对, 三对)}

\statement{在以下分数中, 找出相等的\ch .

<p><center><table border=0>
#if EQCNT ==1
 <tr><td align=right>相等的分数:
 <td>\embed{r1,\size} <td> = <td>\embed{r2,\size}
#else
 <tr><td align=right>第一对:<td>\embed{r1,\size} <td> = <td>\embed{r2,\size}
 <tr><td align=right>第二对:<td>\embed{r3,\size} <td> = <td>\embed{r4,\size}
# if EQCNT > 2
  <tr><td align=right>第三对:<td>\embed{r5,\size} <td> = <td>\embed{r6,\size}
# endif
#endif
</table></center>
}
